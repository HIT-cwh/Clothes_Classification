...
class Visualizer():
    def __init__(self, opt):
        ...
        self.name = opt.name
        self.last_img_dir = os.path.join(opt.checkpoints_dir, opt.name, 'last_images')
        util.mkdirs(self.last_img_dir)
    ...
    def save_last_images(self, visuals, im_path):
        i = 0
        part = im_path[i].split('/')
        name = part[-1].split('.')
        for label, image_numpy in visuals.items():
            img_path = os.path.join(self.last_img_dir, '%s_%s.jpg' % (name[0], label))
